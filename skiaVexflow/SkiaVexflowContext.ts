...
import { createFont, getAdvanceWidth, parseCssFontShorthand, toPxFontSize } from './utils';
...
export default class SkiaVexflowContext implements VexflowRenderContext {
  ...
  setLineCap(capType: CanvasLineCap) {
    const cap: LineCap = capType === 'round' || capType === 'square' ? capType : 'butt';
  ...
  set fillStyle(style: string | CanvasGradient | CanvasPattern) {
    if (typeof style === 'string') {
      ...
      this.fillPaint.setColor(Skia.Color(style));
      return;
    }
  ...
  set strokeStyle(style: string | CanvasGradient | CanvasPattern) {
    if (typeof style === 'string') {
      ...
      this.strokePaint.setColor(Skia.Color(style));
      return;
    }
  ...
  setFont(
    ...
    if (typeof font === 'object' && font !== null) {
      ...
      const family = Array.isArray(font.family) ? font.family.join(', ') : font.family;
      const fontWeight = font.weight ? ` ${font.weight}` : '';
      const fontStyle = font.style ? ` ${font.style}` : '';

      this.font = `${fontStyle}${fontWeight} ${fontSize}px ${family ?? 'sans-serif'}`.trim();
      return this;
    }
