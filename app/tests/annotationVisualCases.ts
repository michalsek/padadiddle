...
import { type VisualCase } from '@/app/tests/accidentalVisualCases';
...
function configureTextMetricsForRn() {
  if (textMetricsConfigured) return;
...
function setAnnotationFontDefaults(
  family: string,
  size: number,
...
function addAnnotation(note: StaveNote | TabNote, spec: AnnotationSpec) {
  ...
  annotation.setJustification(spec.h ?? AnnotationHorizontalJustify.CENTER_STEM);
...
function drawTabVoice(ctx: VexflowCanvasDrawArgs['ctx'], stave: TabStave, notes: TabNote[]) {
  configureTextMetricsForRn();
...
function placementNotes(): StaveNote[] {
  const annotation = (text: string, fontSize: number, vj: AnnotationVerticalJustify) =>
...
export const ANNOTATION_VISUAL_CASES: VisualCase[] = [
  ...
  },
  {
    ...
    draw: ({ ctx, width }) => {
      ...
      drawTabVoice(ctx, new TabStave(10, 10, width - 20).addTabGlyph(), [noteA, noteB]);
    },
  },
  {
    ...
    draw: ({ ctx, width }) => {
      const notes = [
        ...
        addAnnotation(new StaveNote({ keys: ['c/4', 'e/4', 'c/5'], duration: 'h' }), {
          text: 'Allegro',
          index: 2,
          fillStyle: '#2563eb',
        }),
  ...
  },
  {
    ...
    draw: ({ ctx, width }) => {
      ...
      const noteB = new TabNote({
        ...
        .addModifier(new Annotation('(8va)').setFont('Arial', 12, 'normal', 'italic'), 0)
        .addModifier(new Annotation('A.H.'), 0);

      drawTabVoice(ctx, new TabStave(10, 10, width - 20).addClef('tab'), [noteA, noteB]);
    },
  },
  {
    ...
    draw: ({ ctx, width }) => {
      ...
      const notes = [
        ...
        annotateTop(new TabNote({ positions: [{ str: 6, fret: 9 }], duration: '8' }), 'p', 0),
        annotateTop(new TabNote({ positions: [{ str: 3, fret: 9 }], duration: '8' }), 'i'),
        annotateTop(new TabNote({ positions: [{ str: 2, fret: 9 }], duration: '8' }), 'm'),
        annotateTop(new TabNote({ positions: [{ str: 1, fret: 9 }], duration: '8' }), 'a'),
      ];
    ...
    draw: ({ ctx, width }) => {
      const mk = (key: string, text: string) =>
        new StaveNote({ keys: [key], duration: 'w' }).addModifier(
          new Annotation(text).setVerticalJustification(AnnotationVerticalJustify.BOTTOM),
          0
        );
  ...
  },
  {
    ...
    draw: ({ ctx, width }) => {
      const notes = ['good', 'even', 'under', 'beam'].map((word, i) =>
        new StaveNote({ keys: [['a/3', 'g/3', 'c/4', 'd/4'][i]], duration: '8' }).addModifier(
          new Annotation(word).setVerticalJustification(AnnotationVerticalJustify.BOTTOM),
          0
        )
      );
  ...
  },
  {
    ...
    draw: ({ ctx, width }) => {
      for (let v = 1; v <= 4; v++) {
        ...
        drawStaveVoice(ctx, new Stave(16, (v - 1) * 170 + 30, width - 32), notes);
      }
    },
  },
  {
  ...
  },
  {
    ...
    beforeDraw: ({ ctx }) => {
      ...
      // setAnnotationFontDefaults(family, size, weight, style);
    },
    draw: ({ ctx, width }) => {
