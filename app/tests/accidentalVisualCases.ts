...
import { type VexflowCanvasDrawArgs } from '@/skiaVexflow';
...
function formatAndDrawSingleVoice(
  ctx: VexflowCanvasDrawArgs['ctx'],
  width: number,
  notes: StaveNote[],
  options?: { keySignature?: string; x?: number; y?: number }
) {
  const stave = new Stave(options?.x ?? 16, options?.y ?? 40, width - 32).addClef('treble');
...
function drawAccidentalBoundingBoxes(ctx: VexflowCanvasDrawArgs['ctx'], notes: StaveNote[]) {
  ctx.save();
...
export const ACCIDENTAL_VISUAL_CASES: VisualCase[] = [
  {
    ...
    draw: ({ ctx, width }) => {
      ...
      const notesA = make(['bb/4', 'bb/4', 'g#/4', 'g/4', 'b/4', 'b/4', 'a#/4', 'g#/4']);
      const notesB = make(['e#/4', 'cb/4', 'fb/4', 'b#/4', 'b#/4', 'cb/5', 'fb/5', 'e#/4']);
      const notesC = make([
        'c/4',
        'cb/4',
        'cb/4',
      ...
      const staveA = new Stave(16, 20, width - 32).addClef('treble').addKeySignature('F');
      staveA.setContext(ctx as any).draw();
      const voiceA = new Voice({ numBeats: 4, beatValue: 4 }).setStrict(false).addTickables(notesA);
      ...
      const staveB = new Stave(16, 95, width - 32).addClef('treble').addKeySignature('A');
      staveB.setContext(ctx as any).draw();
      const voiceB = new Voice({ numBeats: 4, beatValue: 4 }).setStrict(false).addTickables(notesB);
      ...
      const staveC = new Stave(16, 170, width - 32).addClef('treble').addKeySignature('C');
      staveC.setContext(ctx as any).draw();
      const voiceC = new Voice({ numBeats: 4, beatValue: 4 }).setStrict(false).addTickables(notesC);
  ...
  },
  {
    ...
    draw: ({ ctx, width }) => {
      const notes = [
        ...
        buildNote({ keys: ['e/4', 'f/4', 'a/4', 'c/5', 'e/5', 'g/5', 'd/4'], duration: '2' }, [
          { type: '##', index: 6 },
          { type: 'n', index: 0 },
          { type: 'bb', index: 1 },
          { type: 'b', index: 2 },
        ...
        buildNote({ keys: ['g/5', 'f/4', 'g/4', 'a/4', 'b/4', 'c/5', 'e/5'], duration: '16' }, [
          { type: 'n', index: 1 },
          { type: '#', index: 2 },
          { type: '#', index: 3 },
          { type: 'b', index: 4 },
        ...
        buildNote({ keys: ['a/3', 'c/4', 'e/4', 'b/4', 'd/5', 'g/5'], duration: '1' }, [
          { type: '#', index: 0 },
  ...
  },
  {
    ...
    draw: ({ ctx, width }) => {
      ...
      stave.setContext(ctx as any).draw();
      const voice = new Voice({ numBeats: 4, beatValue: 4 }).setStrict(false).addTickables(notes);
      new Formatter({ softmaxFactor: 100 }).joinVoices([voice]).formatToStave([voice], stave);
      voice.draw(ctx as any, stave);
    },
  },
  {
  ...
  },
  {
    ...
    canvasHeight: 900,
    draw: ({ ctx, width }) => {
      const accids = genAccidentals().filter((item) => item !== '{' && item !== '}');
      const staveCount = 12;
      ...
      for (let row = 0; row < staveCount; row++) {
        ...
        const rowAccids = accids.slice(row * notesPerStave, (row + 1) * notesPerStave);
  ...
  },
  {
    ...
    draw: ({ ctx, width }) => {
      const notes = [
        ...
        buildNote({ keys: ['b/3', 'e/4', 'a/4', 'd/5', 'g/5'], duration: '8' }, [
          { type: 'bb', index: 0 },
          { type: 'b', index: 1, cautionary: true },
  ...
  },
  {
    ...
    draw: ({ ctx, width }) => {
      ...
      drawPair(
        buildNote({ keys: ['c/4', 'e/4', 'a/4'], duration: '2', stemDirection: -1 }, [
          { type: 'b', index: 0 },
          { type: 'n', index: 1 },
          { type: '#', index: 2 },
        ]),
        buildNote({ keys: ['d/5', 'a/5', 'b/5'], duration: '2', stemDirection: 1 }, [
          { type: 'b', index: 0 },
          { type: 'bb', index: 1 },
          { type: '##', index: 2 },
        ]),
      ...
      drawPair(
        buildNote({ keys: ['c/4', 'e/4', 'c/5'], duration: '2', stemDirection: -1 }, [
          { type: 'b', index: 0 },
          { type: 'n', index: 1 },
          { type: '#', index: 2 },
        ]),
        buildNote({ keys: ['d/5', 'a/5', 'b/5'], duration: '4', stemDirection: 1 }, [
          { type: 'b', index: 0 },
        ]),
        300
      );
      ...
      drawPair(
        buildNote({ keys: ['d/4', 'c/5', 'd/5'], duration: '2', stemDirection: -1 }, [
          { type: 'b', index: 0 },
          { type: 'n', index: 1 },
          { type: '#', index: 2 },
        ]),
  ...
  },
  {
    ...
    draw: ({ ctx, width }) => {
      const notes = [
        ...
        buildNote({ keys: ['d/4', 'e/4', 'f/4', 'a/4', 'c/5', 'e/5', 'g/5'], duration: '2' }, [
          { type: 'bbs', index: 0 },
          { type: '++', index: 1 },
          { type: '+', index: 2 },
          { type: 'd', index: 3 },
        ...
        buildNote({ keys: ['f/4', 'g/4', 'a/4', 'b/4', 'c/5', 'e/5', 'g/5'], duration: '16' }, [
          { type: '++', index: 0 },
          { type: 'bbs', index: 1 },
          { type: '+', index: 2 },
          { type: 'b', index: 3 },
        ...
        buildNote({ keys: ['f/4', 'g/4', 'a/4', 'b/4', 'd/5', 'g/5'], duration: '16' }, [
          { type: '++-', index: 0 },
          { type: '+-', index: 1 },
          { type: 'bs', index: 2 },
          { type: 'bss', index: 3 },
  ...
  },
  {
    ...
    canvasWidth: 850,
    draw: ({ ctx, width }) => {
      const notes = ['c/4', 'd/4', 'e/4', 'f/4', 'g/4', 'a/4', 'b/4', 'c/5'].map((pitch) =>
        buildNote({ keys: [pitch], duration: '4' })
      );
  ...
  },
  {
    ...
    canvasWidth: 850,
    draw: ({ ctx, width }) => {
      const notes = ['a/4', 'b/4', 'c#/5', 'd/5', 'e/5', 'f#/5', 'g#/5', 'a/5'].map((pitch) =>
        buildNote({ keys: [pitch], duration: '4' })
      );
      const stave = new Stave(16, 40, width - 32).addClef('treble').addKeySignature('A');
      stave.setContext(ctx as any).draw();
      const voice = new Voice({ numBeats: 4, beatValue: 4 }).setStrict(false).addTickables(notes);
  ...
  },
  {
    ...
    canvasWidth: 900,
    draw: ({ ctx, width }) => {
      const notes0 = ['c/4', 'd/4', 'e/4', 'f/4', 'g/4', 'a/4', 'b/4', 'c/5'].map((pitch) =>
        buildNote({ keys: [pitch], duration: '4', stemDirection: -1 })
      );
      const notes1 = ['c/5', 'd/5', 'e/5', 'f/5', 'g/5', 'a/5', 'b/5', 'c/6'].map((pitch) =>
        buildNote({ keys: [pitch], duration: '4' })
      );
      const stave = new Stave(16, 40, width - 32).addClef('treble').addKeySignature('Ab');
      stave.setContext(ctx as any).draw();
      const voice0 = new Voice({ numBeats: 4, beatValue: 4 }).setStrict(false).addTickables(notes0);
      const voice1 = new Voice({ numBeats: 4, beatValue: 4 }).setStrict(false).addTickables(notes1);
  ...
  },
  {
    ...
    draw: ({ ctx, width }) => {
      ...
      const notes1 = ['c/5', 'c/5', 'd/5', 'e/5', 'f/5', 'g/5', 'a/5', 'b/5', 'c/6'].map(
        (pitch, index) => buildNote({ keys: [pitch], duration: index === 0 ? '8' : '4' })
      );
      const stave = new Stave(16, 40, width - 32).addClef('treble').addKeySignature('Cb');
      stave.setContext(ctx as any).draw();
      const voice0 = new Voice({ numBeats: 4, beatValue: 4 }).setStrict(false).addTickables(notes0);
      const voice1 = new Voice({ numBeats: 4, beatValue: 4 }).setStrict(false).addTickables(notes1);
  ...
  },
  {
    ...
    canvasWidth: 1000,
    draw: ({ ctx, width }) => {
      const notes = ['c/4', 'c#/4', 'c#/4', 'c/4', 'c/4', 'cb/4', 'cb/4', 'c/4', 'c/4'].map(
        (pitch) => buildNote({ keys: [pitch], duration: '4', stemDirection: -1 })
      );
      const stave = new Stave(16, 40, width - 32).addClef('treble').addKeySignature('C');
      stave.setContext(ctx as any).draw();
      const voice = new Voice({ numBeats: 4, beatValue: 4 }).setStrict(false).addTickables(notes);
      Accidental.applyAccidentals([voice], 'C');
      new Formatter().joinVoices([voice]).formatToStave([voice], stave);
  ...
  },
  {
    ...
    draw: ({ ctx, width }) => {
      const notes = [
        ...
        'c/4',
        'c/5',
      ].map((pitch) => buildNote({ keys: [pitch], duration: '4', stemDirection: -1 }));
      const stave = new Stave(16, 40, width - 32).addClef('treble').addKeySignature('C');
      stave.setContext(ctx as any).draw();
      const voice = new Voice({ numBeats: 4, beatValue: 4 }).setStrict(false).addTickables(notes);
      Accidental.applyAccidentals([voice], 'C');
  ...
  },
  {
    ...
    draw: ({ ctx, width }) => {
      const notes = [
        ...
        buildNote({ keys: ['d/4', 'e/4', 'f/4', 'a/4', 'c/5', 'e/5', 'g/5'], duration: 'h' }, [
          { type: '##', index: 0 },
          { type: 'n', index: 1 },
          { type: 'bb', index: 2 },
          { type: 'b', index: 3 },
        ...
        buildNote({ keys: ['a/3', 'c/4', 'e/4', 'b/4', 'd/5', 'g/5'], duration: 'w' }, [
          { type: '#', index: 0 },
          { type: '##', index: 1, cautionary: true },
          { type: '#', index: 2, cautionary: true },
          { type: 'b', index: 3 },
